...
@onready var _display_name = $Panel/VBoxContainer/Connect/DisplayName
...
func _on_connect_toggled(pressed):
	if not pressed:
		_client.close()
		return
	if _host.text == "" and _display_name.text == "":
		return
...
func _process(delta):
	var messages = _client.poll()
	if messages:
		# ユーザーのディスプレイ名登録の通信を送る
		if "Connection Successfull!" in messages:
			info("%s Registered." % [ _display_name.text])
			_client.send("Register Display Name:" + _display_name.text)
		else:
			info(messages)
